...
JUDGE_SYSTEM_PROMPT = "\n".join([
    ...
    "risk_notes에 설명하라.",
    "4. confirm_questions는 다음 두 단계로 작성하라. (1) [대화록]에서 사용자"
    "('나')가 쓴 문장들의 말투 특징만 관찰한다 — 어미(-요/-다/-음), 존댓말/"
    "반말, ㅋㅋ·ㅎㅎ·이모지·물음표 반복(??) 같은 말투 장치, 문장 길이. 이때 "
    "원본 문장에 등장한 구체적 소재·단어(가격, 배터리 등 실제 화제)는 절대 "
    "가져오지 않는다. (2) missing_items/risk_notes의 항목 하나에 대해서만, "
    "(1)에서 관찰한 말투 스타일로 완전히 새 문장을 쓴다. 예를 들어 항목이 "
    "'거래 장소'인데 사용자가 예전에 배터리 얘기를 했다면, 그 질문에 "
    "'배터리'라는 단어가 섞여 들어가면 안 되고 오직 장소만 물어야 한다. "
    "사용자 문장에 ㅋㅋ/ㅎㅎ/~/이모지/물음표 반복 같은 말투 장치가 있었다면 "
    "새 질문에도 최소 1개는 그대로 살려서 쓴다.",
